...
export default async function CalculatorPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schema = { "@context": "https://schema.org", "@graph": [{ "@type": "WebApplication", "@id": `${pageUrl}#application`, name: config.title, url: pageUrl, applicationCategory: "FinanceApplication", operatingSystem: "Any", isAccessibleForFree: true, offers: { "@type": "Offer", price: "0", priceCurrency: "USD" }, description: config.description, provider: { "@id": absoluteUrl("/#organization") } }, { "@type": "FAQPage", "@id": `${pageUrl}#faq`, url: pageUrl, mainEntity: config.faqs.map(([question, answer]) => ({ "@type": "Question", name: question, acceptedAnswer: { "@type": "Answer", text: answer } })) }] };
