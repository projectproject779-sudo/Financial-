...
export default function GuidesPage() { return <><SiteHeader /><main id="main-content"><section className="listing-hero"><div className="shell listing-hero-inner"><div><p className="eyebrow">Evidence before opinion</p><h1>Money guides built for decisions, not clicks.</h1><p>Plain-English explanations, transparent assumptions, primary sources, and practical checklists.</p></div><div className="listing-stat"><strong>{guides.length + originals.length}</strong><span>in-depth guides published by founder Shuxrat Asliddinov</span></div></div></section><section className="directory-section"><div className="shell guide-library">{[...originals, ...guides.map((guide) => ({ href: `/insights/${guide.slug}`, category: guide.category, title: guide.title, copy: guide.description }))].map((guide, index) => <article key={guide.href}><span>{String(index + 1).padStart(2, "0")} · {guide.category}</span><h2>{guide.title}</h2><p>{guide.copy}</p><Link href={guide.href}>Read guide →</Link></article>)}</div></section></main><SiteFooter /></>; }
